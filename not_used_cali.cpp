...
void mdrawingCircle(Mat img, InputArray _rvec, InputArray _tvec, InputArray _K, InputArray _D)
{
    ...
    for(double fov=0; fov <= 90; fov=fov+10)
    {
        double theta = fov * CV_PI/ 180.0 ;
        ...
        for(double fov2=0; fov2<360; fov2++)
        {
            double phi = fov2 * CV_PI / 180.;
            double x_ = cdist * r * cos(phi);
            double y_ = cdist * r * sin(phi);
            Vec3d xd5(x_, y_, 1);
            // Vec3d Y = aff*xd1;
            // Vec2d xd3(Y[0]/Y[2], Y[1]/Y[2]);
            // Vec2d xd5 = xd3;
            // cout << xd5 << endl;
            Vec2i final_point(cvRound(xd5[0] * f[0] + c[0]), cvRound(xd5[1] * f[1] + c[1]));
            
            cv::circle(img, final_point, 1, Scalar(255,30,30), -1, cv::LINE_AA);
...
Mat mCorrection(Mat img, InputArray _rvec, InputArray _tvec, InputArray _K, InputArray _D)
{
    CV_Assert(_rvec.total() * _rvec.channels() == 3 && (_rvec.depth() == CV_32F || _rvec.depth() == CV_64F));
    CV_Assert(_tvec.total() * _tvec.channels() == 3 && (_tvec.depth() == CV_32F || _tvec.depth() == CV_64F));
    CV_Assert(_tvec.getMat().isContinuous() && _rvec.getMat().isContinuous());
    ...
    Mat corimg = cv::Mat::zeros(viewSize, img.type());

    for(int h=0; h< viewSize.height; h++)
    {
        double h_ = h - c[0];

        for(int w=0; w<viewSize.width; w++)
        {
            double w_ = w - c[1];
            ...
            double theta = atan2(h_,w_);
            double r = sqrt(w_*w_+h_*h_);
            ...
            double x_ = cdist * r * w_;
            double y_ = cdist * r * h_;
            Vec3d xd1(x_, y_, 1);
            Vec3d Y = aff*xd1;
            ...
            Vec2i final_point(cvRound(xd5[0] * f[0] + viewSize.width/2), cvRound(xd5[1] * f[1] + viewSize.height/2));

            // cout << "final_point : " << final_point << endl;

            if (0<=final_point[0] && 0<=final_point[1] && final_point[1]< h && final_point[0] < w)
                corimg.at<uchar>(h,w) = img.at<uchar>(final_point);    
        }
    }
...
int main(int argc, char** argv)
{
    ...
    {
        Mat result_ = mCorrection(viewlist[i], rvec[i], tvec[i], cameraMatrix, distCoeffs);
        cv::resize(result_, result_, Size(1000,1000));
        imshow("test", result_);
        waitKey(0);
    ...
    {   
        ...
        fisheye::projectPoints(cv::Mat(objectPoints[i]), rep_corner_pts, rvec[loc], tvec[loc], cameraMatrix, distCoeffs);
    ...
    cout << "-------------Mean Reprojection error (Scaramuzza)--------------" << endl;
    cout << tot_error << endl;
